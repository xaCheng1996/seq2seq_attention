...
class Seq2SeqModel():
    def __init__(self, rnn_size, num_layers, embedding_size, learning_rate, word_to_idx, mode, use_attention,
                 beam_search, beam_size, embedding_matrix, max_gradient_norm=5.0,):
    ...
    def build_model(self):
        ...
        # =================================定义一个数值x，指定为最低的loss:
        self.loss_min = tf.Variable(initial_value=500.0)
        #=================================2, 定义模型的encoder部分
        with tf.variable_scope('encoder'):
            # embedding矩阵，来自data_load.embedding_re()的加载
            embedding = tf.constant(value=self.embedding_matrix, shape=(self.vocab_size, self.embedding_size),
                                    dtype=tf.float32)
            embedding = tf.Variable(initial_value=embedding, name="embedding")
            self.embedding = embedding
            encoder_inputs_embedded = tf.nn.embedding_lookup(embedding, self.encoder_inputs)
            encoder_cell = self._create_rnn_cell()
            # 使用dynamic_rnn构建LSTM模型，将输入编码成hidden state。
        ...
        with tf.variable_scope('decoder'):
            ...
            # 这里定义了两种attention_mechanism 选择自己需要的使用即可
            ...
            # 定义decoder阶段用的LSTMCell，然后为其封装attention wrapper
            ...
            if self.mode == 'train':
                ...
                #训练阶段，使用TrainingHelper+BasicDecoder的组合，这一般是固定的，当然也可以自己定义Helper类
                ...
                if self.beam_search:
                    ...
                    inference_decoder = tf.contrib.seq2seq.BasicDecoder(cell=decoder_cell, helper=decoding_helper,
                                                                        initial_state=decoder_initial_state,
                                                                        output_layer=output_layer)
                decoder_outputs, _, _ = tf.contrib.seq2seq.dynamic_decode(decoder=inference_decoder, maximum_iterations=15)
    ...
    def train(self, sess, batch):
        ...
        _, loss, summary, pred = sess.run([self.train_op, self.loss, self.summary_op, self.decoder_predict_train], feed_dict=feed_dict)
